...
In order to use this `run-operator-playbook` target, you must have Python3 in your PATH (having an alias is not enough). You must also `python -m pip install --user --upgrade -r operator/molecule/requirements.txt` to get the necessary Python3 libraries installed/upgraded.
