...
- [ ] If you modified the CRD schema, you must sync the CRD changes. This requires both an operator PR and a helm chart PR since the operator helm chart also needs the updated CRD. Use the `make sync-crds` target in the kiali-operator repository to synchronize CRD changes to all necessary locations.
...
- [ ] If a CRD property was added, removed, or modified, update the link:./molecule/config-values-test/kiali-cr-all-yaml[comprehensive molecule CR]
