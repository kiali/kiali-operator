...
- [ ] Document the new value in the link:./crd-docs/crd/kiali.io_kialis.yaml[CRD schema] (and put the default values in the link:./crd-docs/cr/kiali.io_v1_kiali.yaml[example CR]). Run `make validate-cr` in your local kiali-operator repo to ensure your changes are valid.
